...
public class Hashtable<K,V>
     ...
     * The hash table data.
     */
    private transient Entry<?,?>[] table; //用于存放系统键值对

    /**
     * The total number of entries in the hash table.
     */
    private transient int count; //哈希表条目总数
     ...
     * @serial
     */
    private int threshold; //记录哈希表大小的阈值
     ...
     * @serial
     */
    private float loadFactor; //哈希表的加载因子，装填因子
    ...
    protected void rehash() {
        int oldCapacity = table.length; //旧表长度
        Entry<?,?>[] oldMap = table; //获取全部键值对

        // overflow-conscious code
        int newCapacity = (oldCapacity << 1) + 1; //新表长度等于旧表长度+2+1
        if (newCapacity - MAX_ARRAY_SIZE > 0) { //若是新表长度大于数组的最大长度
            if (oldCapacity == MAX_ARRAY_SIZE) //旧表已经是最大了
                // Keep running with MAX_ARRAY_SIZE buckets
                return; //无法继续扩大，直接返回
            newCapacity = MAX_ARRAY_SIZE; //扩大到允许的最大
        }
        Entry<?,?>[] newMap = new Entry<?,?>[newCapacity]; //建立新表

        modCount++; //哈希表操作计数器
        threshold = (int)Math.min(newCapacity * loadFactor, MAX_ARRAY_SIZE + 1); //loadFactor 哈希表加载（装填）因子，赋值哈希表的长度
        table = newMap; //赋值新表

        //重新为新表赋值
        for (int i = oldCapacity ; i-- > 0 ;) { //遍历旧表长度
            for (Entry<K,V> old = (Entry<K,V>)oldMap[i] ; old != null ; ) { //遍历旧表实例
                Entry<K,V> e = old; //记录数据
                old = old.next; //记录下一位数据

                int index = (e.hash & 0x7FFFFFFF) % newCapacity; //哈希槽定位，计算位置
                e.next = (Entry<K,V>)newMap[index]; //链表插入，下一位指向原本该位的键值对
                newMap[index] = e; //为该位赋值
            }
        }
    }

...
public final class System {
     ...
     */
    //该方法为native函数，使用c语言实现，请查看md文档详细介绍
    ...
    public static String setProperty(String key, String value) {
        checkKey(key); //查验key值合法性
        SecurityManager sm = getSecurityManager(); //存在安全框架则获得，没有则为null
        if (sm != null) { //当sm不为null时
            sm.checkPermission(new PropertyPermission(key,
                SecurityConstants.PROPERTY_WRITE_ACTION));
            //查验write的权限
        }

        return (String) props.setProperty(key, value); //调用小工具里的set函数来创建
    }
